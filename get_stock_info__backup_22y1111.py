...
def get_st_info(st_type = ['DJI','INDEX','HDI', 'HKI']):
    ...
    #다우지수
    st_DJI = stock.get_index_ohlcv_by_date(s_date,e_date,"1001",name_display=True)
    st_DJI['당일_변화율'] = st_DJI.loc[:,['종가']].pct_change() * 100
    ...
    text_DJI_Title = '다우지수'
    text_DJI_Title = text_DJI_Title + '\n 종가 : {}원 ({:0.1f}%)'.format(st_DJI.iloc[-1,0],st_DJI.iloc[-1,-1])
    ...
    text_INDEX_Title = text_INDEX_Title + '\n 종가 : {}원 ({:0.1f}%)'.format(st_KOSPI.iloc[-1,0],st_KOSPI.iloc[-1,-1])
